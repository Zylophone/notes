...
- Encode the value with an encoding function:
  - base 64, base 32, hexadecimal?
  - transform value from original representation (binary) to target representation (characters)
- Final key would be 6 or 8 chars long
  - 64**6 || 64**8 possible different strings
- Base 64
  - Each character can take 64 different values
  - If I use base 10, then I would need two integers to represent each char.
    - 2 bytes = 16 bits
  - If I use base 2, then I would need log2(64) = 6 bits of data
  - why the encoding process group 3 bytes to perform the encoding?
    - 3*8 = 24 % 3 = 0 :)
    - 2*8 = 16 % 3 = 1 :(
    - 1*8 = 8 % 3 = 2 :(
    - If binary stream is not of the given length, then 0s are added so the downstream process can continue without worrying about this edge case
  - Overhead factor
    - the overhead ratio is 4:3
      - 4 bytes as output, 3 bytes as input
      - 3 bytes are divided by 6 bits and the resulting chars are 4 and they need to be stored by using 4 bytes.
** Capacity Estimation ($)
- ratio between reads and writes
  - this helps to determine whether the system is read or write intensive
  - always put the higher number first (4:3)
- traffic
  - network input
  - network output
- disk storage
  - should we keep records for ever?
  - can we delete/reuse them?
  - calculate this per second and times the total years
- memory storage (cache)
  - 80%, 20% Pareto's law
    - 80% of the traffic come from 20% of the urls
  - calculate this per day
* Databases
...
def connect(root)
  marker = TreeLinkNode.new("#")
  queue = [root, marker]
  while !queue.empty?
    current = queue.shift
    case current.val
    when "#"
      queue << marker if !queue.empty?
    else
      current.next = queue[0] if queue[0] != marker
...
def connect_2(root)
  connect_internal(root)
  connect_external(root.left, root.right)
end

def connect_internal(current)
  return if current.left.nil? || current.right.nil?

  current.left.next = current.right
  connect_internal(current.left)
  connect_internal(current.right)
end

def connect_external(left, right)
...
#+BEGIN_SRC ruby
matrix = [[],[],[]]
1.- sort matrix by row
2.- check that columns are sorted in lexicagraphical order
3.- if yes, output yes, output no otherwise
#+END_SRC
** System Design
*** Web Crawler
#+BEGIN_SRC ruby
class URLStore
  attr_accessor :hash, :lock
  def initialize
    self.hash = {}
    self.lock = Mutex.new
  end
  ...
  def add(url_str)
    self.lock.synchronize do
      if !self.hash.has_key?(url_str)
        self.hash[url_str] = URL.new(url_str)
      end
      self.hash[url_str]
    end
  end
...
class URL
  attr_accessor :url, :lock, :visited
...
def give_me_more_urls(url)
  []
end

def process(url_str, store)
  if !current_url = store.get(url_str)
    current_url = store.add(url_str)
  end

  current_url.lock.synchronize do
    return if current_url.visited
    current_url.visited = true
  end
  url_strs = give_me_more_urls(url)
  threads = url_strs.map do |url_str|
    Thread.new { process(url_str, store) }
  end
  threads.map(&:join)
end

def main(root_url_str)
  process(root_url_str, URLStore.new)
end
#+END_SRC

*** Elevator in Multi-stored Aparment
http://massivetechinterview.blogspot.com/2015/07/thought-works-object-oriented-design.html
#+BEGIN_SRC ruby
class Elevator
  attr_accessor :buttons, :state, :current_floor
  def move
  end
...
class Button
  attr_accessor :state, :direction, :elevator_request_handler
  def iluminate
    self.state = :on
  end

  def do_not_iluminate
    self.state = :off
  end

  def request
    self.elevator_request_handler.schedule(self.direction)
  end
end
...
class ElevatorController
  attr_accessor :elevator, :elevator_request_handler
  def initialize(elevator, elevator_request_handler)
    self.elevator = elevator
    self.elevator_request_handler = elevator_request_handler
  end

  def start_elevator
  end

  def shutdown_elevator
  end

  def move_elevator
    request = self.elevator_request_handler.next_request
    self.elevator.move(request.floor)
  end
end

class ElevatorRequestHandler
  attr_accessor :requests, :scheduling_strategy
  ...
  def next_request
    self.scheduling_strategy.call(requests)
  end
end
#+END_SRC
*** Key Generation Service
- Generates Key Off-line.
- Give a Unique Non Used Key to a Client.
- 64**6 different keys
  - 64**6 * 6 bytes ~= 412GB of storage
- Storage
  - SQL
    - Postgres supports table size up until 32TB [[https://www.postgresql.org/about/][(Size Documentation)]]
    - Master / Slave setup for redundancy
    - Key Generation (write) interacts with master instance
      - There are no problems with uniqueness
    - Take Key (write) interact with master instance
      - Lock mechanism needed to guartantee that each request gets a unique url
        - Explicit Locking [[https://www.postgresql.org/docs/current/static/explicit-locking.html][(More Info)]]
        - FOR UPDATE SKIP LOCKED [[https://dba.stackexchange.com/questions/69471/postgres-update-limit-1][(More Info)]]
        - No partition or sharding needed
  - NoSQL
    - Dynamo or Cassandra
    - Sharding is needed
    - Key Generation (write) will pass the newly generated key to a hash function to determine which DB node will hold the value
      - What would happen with uniqueness if hosts are added/lost. Will consistent hash help us?
      - Sequential lookup needed in order to ensure there is no duplication?
        - what would happen when a host that had a key is not available but it become available afterwards?
        - sharing keys might help?
      - should we have different process enqueueing generated keys and only one process writing to the DB?
    - Take Key (write) will interact with a loadbalancer (assigns shard in a round robin fashion)
      - I don't see a way to assign a particular shard to the request other than a random access (round robin?)
      - Optimist Locking
        - Client has to read first in order to get a version of the record
          - This read has to go throught the load balancer
        - Then update it [[http://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_UpdateItem.html][(Optimist Locking)]] [[http://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_UpdateItem.html][(Update)]]
          - This write has to go throught the hash function to pick the correct DB node that holds the record
* Learnings
...
4      20
 ...
 5 8     12
  7
*** Negative
- I made assumptions about duplication
- I missed that O(log n) is only when BST is balanced
- I couldn't come with meaningful examples
- What does it mean to have access to the root
- Visualize it more
- Too specific -> try to be more generic in the solution
*** Positive
+ brute force solution didn't impress the interviewer
+ clarificatinos about the imput
